...
import { getVinylCategory, setSearchRecordTerm, setVinylCategory } from '../../redux/VinylRecordSlice';
...
const CategoryBtn = ({ itemIndex, musicCategory, setMusicCategory, setEditingMusicCategory, updatingInCategoryInput, deleteMusicCategory }) => {
    ...
    const handleFilterProjectCategory = () => {
        dispatch(setVinylCategory(musicCategory))
        dispatch(setSearchRecordTerm(''))
    };
    ...
    return (
        <div>
            <div className="category_button_wrapper">
                <button 
                    className={ `category_button ${ vinylCategory === musicCategory ? "active" : "" }` } 
                    type="button"
                    onClick={ handleFilterProjectCategory }
                >
                    { musicCategory }
                </button>
                <div className={ `category_indicator ${ vinylCategory === musicCategory ? "active" : "" }` } />
            </div>
            { isAdmin && 
